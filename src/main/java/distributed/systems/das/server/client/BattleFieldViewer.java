...
public class BattleFieldViewer extends JPanel implements Runnable {
	...
	public void paint(Graphics g) {
		// Unit u;
		double x = 0, y = 0;
		double xRatio = (double)this.getWidth() / 25;//(double)BattleField.MAP_WIDTH;
		double yRatio = (double)this.getHeight() / 25;//(double)BattleField.MAP_HEIGHT;
		double filler;
		...
		for(int i = 0; i < 25; i++, x += xRatio, y = 0)
			//for(int j = 0; j < BattleField.MAP_HEIGHT; j++, y += yRatio) {
			for(int j = 0; j < 25; j++, y += yRatio) {
				/*u = bf.getUnit(i, j);
				...
				filler = (double)yRatio * 50 / (double)100;
				...
				doubleBufferGraphics.drawString("" + "unitID"/*u.getUnitID()*/, (int)x, (int)y + 15);
				doubleBufferGraphics.setColor(Color.BLACK);
